...
our_data_path_prefix = "/home/user/go/src/github.com/docker/workload/docker_data"
...
def j_send_warmup_thread(requests, q, registry, path_q):
    ...
    for request in requests:
        ...
        if not ( uri in trace):
            ...
            # sha256_hash = uri.split('/')[-1].replace(':','_')
            sha256_hash = uri.split('/')[-1].split(':')[-1]
            # print(sha256_hash)
            our_data_path = f"{our_data_path_prefix}/{our_reponame}/blobs/sha256/{sha256_hash}" 
            try:
...
def warmup(data, out_trace, registry, threads):
    ...
    current_folder = os.environ.get('DEDUP_STORAGE_PATH', '/home/simenc2/docker_v2/docker/registry/v2/blobs')
    # 等待服务器做完dedup
    time.sleep(5)
    try:
    ...
    if now_size > 0:
        ...
        print("nowsize = 0 , dedup ration = N/A (storage folder empty or not found: {})".format(current_folder))
