...
func TestSIG0(t *testing.T) {
	...
	for _, alg := range []uint8{ECDSAP256SHA256, ECDSAP384SHA384, RSASHA1, RSASHA256, RSASHA512} {
		...
		if err != nil {
			t.Errorf("failed to generate key for â€?sâ€? %v", algstr, err)
			continue
		}
		...
		if err != nil {
			t.Errorf("failed to sign message using â€?sâ€? %v", algstr, err)
			continue
		}
		m := new(Msg)
		if err := m.Unpack(mb); err != nil {
			t.Errorf("failed to unpack message signed using â€?sâ€? %v", algstr, err)
			continue
		}
		if len(m.Extra) != 1 {
			t.Errorf("missing SIG for message signed using â€?sâ€?, algstr)
			continue
		}
		var sigrrwire *SIG
		...
		for _, rr := range []*SIG{sigrr, sigrrwire} {
			...
			if err := rr.Verify(keyrr, mb); err != nil {
				t.Errorf("failed to verify â€?sâ€?signed SIG(%s): %v", algstr, id, err)
				continue
			}
		}
		mb[13]++
		if err := sigrr.Verify(keyrr, mb); err == nil {
			t.Errorf("verify succeeded on an altered message using â€?sâ€?, algstr)
			continue
		}
		sigrr.Expiration = 2
		sigrr.Inception = 1
		...
		if err := sigrr.Verify(keyrr, mb); err == nil {
			t.Errorf("verify succeeded on an expired message using â€?sâ€?, algstr)
			continue
		}
	}
}
