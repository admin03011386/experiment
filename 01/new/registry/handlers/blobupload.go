...
func blobUploadDispatcher(ctx *Context, r *http.Request) http.Handler {
	...
	if !ctx.readOnly {
		//		ctxu.GetLogger(ctx).Infof("simenc: blobUploadDispatcher")
	...
	if buh.UUID != "" {
		...
		//		ctxu.GetLogger(ctx).Infof("simenc: blobs.Resume")
...
func (buh *blobUploadHandler) StartBlobUpload(w http.ResponseWriter, r *http.Request) {
	...
	//context.GetLogger(ctx).Infof("simenc: StartBlobUpload: blob object")
	//	log.Warnf("simenc: StartBlobUpload: blob object")
	blobs := buh.Repository.Blobs(buh)
	//context.GetLogger(ctx).Infof("simenc: StartBlobUpload: create blob object")
	//	log.Warnf("simenc: StartBlobUpload: create blob object")
...
func (buh *blobUploadHandler) PatchBlobData(w http.ResponseWriter, r *http.Request) {
	log.Warnf("simenc: PatchBlobData")
	if buh.Upload == nil {
		buh.Errors = append(buh.Errors, v2.ErrorCodeBlobUploadUnknown)
		return
	}
	...
	//	log.Warnf("simenc: PatchBlobData: copyFullPayload")
	...
	//	log.Warnf("simenc: PatchBlobData: blobUploadResponse")
...
func (buh *blobUploadHandler) PutBlobUploadComplete(w http.ResponseWriter, r *http.Request) {
	...
	// simenc: Here, we send a des address to deduplication service
	...
	ctxu.GetLogger(buh).Debugf("simenc: PutBlobUploadComplete: request type: %s, for repo (%s) and usr (%s) with dgst (%s)", reqtype, reponame, usrname, dgst.String())
	go buh.Upload.FileDedup(
		reqtype,
		reponame,
		usrname,
