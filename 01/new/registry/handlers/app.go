...
import (
	...
	"github.com/docker2/new/configuration"
	ctxu "github.com/docker2/new/context"
	"github.com/docker2/new/health"
	"github.com/docker2/new/health/checks"
	"github.com/docker2/new/notifications"
	"github.com/docker2/new/reference"
	"github.com/docker2/new/registry/api/errcode"
	"github.com/docker2/new/registry/api/v2"
	"github.com/docker2/new/registry/auth"
	registrymiddleware "github.com/docker2/new/registry/middleware/registry"
	repositorymiddleware "github.com/docker2/new/registry/middleware/repository"
	"github.com/docker2/new/registry/proxy"
	"github.com/docker2/new/registry/storage"
	memorycache "github.com/docker2/new/registry/storage/cache/memory"
	rediscache "github.com/docker2/new/registry/storage/cache/redis"
	//	redisDedup "github.com/docker2/new/registry/storage/cache/redisDedup"
	storagedriver "github.com/docker2/new/registry/storage/driver"
	"github.com/docker2/new/registry/storage/driver/factory"
	storagemiddleware "github.com/docker2/new/registry/storage/driver/middleware"
...
func NewApp(ctx context.Context, config *configuration.Configuration) *App {
	...
	ctxu.GetLogger(app).Infof("simenc: Register blobDispatcher")
	...
	if cc, ok := config.Storage["cacheparams"]; ok {
		...
		if ok1 && ok2 && ok3 && ok4 && ok5 {
			...
			if ok1 && ok2 && ok3 && ok4 && ok5 {
				...
				panic(fmt.Sprintf("simenc: cannot parse cacheparams: %v, %v, %v, %v, %v", ok1, ok2, ok3, ok4, ok5))
			}
		} else {
			panic(fmt.Sprintf("simenc: cannot parse cacheparams: %v, %v, %v, %v, %v", ok1, ok2, ok3, ok4, ok5))
		}
	...
	if cc, ok := config.Storage["registryparams"]; ok {
		...
		if ok1 && ok2 && ok3 && ok4 && ok5 {
			...
			if ok1 && ok2 && ok3 && ok4 && ok5 {
				...
				panic(fmt.Sprintf("simenc: cannot parse registryparams: %v, %v, %v, %v, %v", ok1, ok2, ok3, ok4, ok5))
			}
		} else {
			panic(fmt.Sprintf("simenc: cannot parse registryparams: %v, %v, %v, %v, %v", ok1, ok2, ok3, ok4, ok5))
		}
	...
	if cc, ok := config.Storage["cache"]; ok {
		...
		//simenc: which one is used for caching blobdescriptor.
		if !ok {
			// Backwards compatible: "layerinfo" == "blobdescriptor"
			v = cc["layerinfo"]
		}
		//simenc: put here for redis and registry

		hostip = fmt.Sprintf("%v", cc["hostip"])
		switch v {
		// simenc: store blob descriptor
		case "redis":
			if app.redis == nil {
				panic("redis configuration required to use for layerinfo cache")
			}

			cacheProvider := rediscache.NewRedisBlobDescriptorCacheProvider(app.redis, app.cluster)
...
func (app *App) configureRedis(configuration *configuration.Configuration) {
	...
	pool := &redis.Pool{
		Dial: func() (redis.Conn, error) {
			...
			//simenc: 30 * 30*configuration.Redis.DialTimeout
	...
	app.redis = pool
	//simenc: add redisc cluster

	connflush := pool.Get()
	connflush.Do("FLUSHALL")
	defer connflush.Close()

	var redisdb *redisgo.ClusterClient
	found := false

	for _, ip := range configuration.Notifications.Registries {
		if "192.168.0.170" == ip {
			...
			fmt.Println(ok, err)
			fmt.Println("==========> THIS IS TESTING HULKS ============> \n")
			found = true
			break
		}
	...
	if false == found {
		...
		ok, err := redisdb.FlushAll().Result()
		fmt.Println(ok, err)
		fmt.Println("==========> THIS IS TESTING THORS ============> \n")
	}
