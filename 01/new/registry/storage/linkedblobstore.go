...
func (lbs *linkedBlobStore) ServeBlob(ctx context.Context, w http.ResponseWriter, r *http.Request, dgst digest.Digest) error {
	//fmt.Printf("simenc: (lbs *linkedBlobStore) ServeBlob dgst: %v\n",dgst)
	canonical, err := lbs.Stat(ctx, dgst) // access check
	//fmt.Printf("simenc: canonical.dgst: %v ,err = %v \n",canonical ,err)
	if err != nil {
		return err
	}
...
//	context.GetLogger(ctx).Debug("simenc: This is for upload(*linkedBlobStore).Writer")
...
//	context.GetLogger(ctx).Debug("simenc: lbs: start upload location is %s", path)
	if err != nil {
		return nil, err
	}
...
//	context.GetLogger(ctx).Debug("simenc: lbs: startedat file is %s", startedAtPath)
	if err := lbs.blobStore.driver.PutContent(ctx, startedAtPath, []byte(startedAt.Format(time.RFC3339))); err != nil {
		return nil, err
	}
//	context.GetLogger(ctx).Debug("simenc: (*linkedBlobStore).Writer: call lbs.newBlobUpload")
...
func (lbs *linkedBlobStatter) SetDescriptor(ctx context.Context, dgst digest.Digest, desc distribution.Descriptor) error {
	...
	//fmt.Println("simenc: func (lbs *linkedBlobStatter) SetDescriptor enter")
	return nil
}
