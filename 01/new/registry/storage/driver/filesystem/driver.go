...
import (
	...
	"github.com/docker2/new/registry/storage/driver/base"
	"github.com/docker2/new/registry/storage/driver/factory"
)

const (
	driverName           = "filesystem"
	defaultRootDirectory = "/home/simenc3/docker_v2"
...
func (d *driver) Stat(ctx context.Context, subPath string) (storagedriver.FileInfo, error) {
	...
	if err != nil {
		...
		if "PRECONSTRUCTLAYER" == context.GetType(ctx) || "PRECONSTRUCTSLICE" == context.GetType(ctx) { //type == preconstruct// skip{}
			context.GetLogger(ctx).Debugf("simenc: driver fs Stat: skip the errors for preconstruct layer or slice for path: %v", subPath)
		} else {
			if os.IsNotExist(err) {
