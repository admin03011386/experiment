...
import (
	...
	"github.com/docker2/new"
	"github.com/docker2/new/context"
	storagedriver "github.com/docker2/new/registry/storage/driver"
	"github.com/docker2/new/version"
	...
	"path/filepath"
	"sync"
	"github.com/docker2/new/registry/storage/cache"
	"bytes"
	"io/ioutil"
	"net/http"
	"math/rand"
	...
	for server, fpathlst := range serverForwardMap {
		...
		go func(server string, fpathlst []string, fwg *sync.WaitGroup) {
			...
			for _, fpath := range fpathlst {
				...
				destfpath := path.Join("simenc_NO_NEED_TO_DEDUP_THIS_TARBALL", strings.TrimPrefix(fpath, "/home/simenc3/docker_v2"))
				wg.Add(1)
				antp.Invoke(&Task{
	...
	for _, f := range files {
		...
		if fmatch {
			fmt.Printf("simenc: NO_NEED_TO_DEDUP_THIS_TARBALL: %s \n", f.Name())
			 /*
			  /home/simenc3/dockerimages/layers
			...
			*/
			...
			for _, f := range files {
				...
				if _, err := os.Stat(path.Join("/home/simenc3/docker_v2/docker/registry/v2/blobs/sha256/", f.Name())); err == nil {
					//path exists
					...
					for _, fd := range fds {
						if err = os.Rename(path.Join(unpackPath, "NO_NEED_TO_DEDUP_THIS_TARBALL/docker/registry/v2/blobs/sha256/", f.Name(), fd.Name()),
							path.Join("/home/simenc3/docker_v2/docker/registry/v2/blobs/sha256/", f.Name(), fd.Name())); err != nil {
					...
					if err = os.Rename(path.Join(unpackPath, "NO_NEED_TO_DEDUP_THIS_TARBALL/docker/registry/v2/blobs/sha256/", f.Name()),
						path.Join("/home/simenc3/docker_v2/docker/registry/v2/blobs/sha256/", f.Name())); err != nil {
						fmt.Printf("simenc: %s, cannot rename this unpackpath filepath: error %s, probaly a deuplicate file, Ignore it \n",
							path.Join(unpackPath, "NO_NEED_TO_DEDUP_THIS_TARBALL/docker/registry/v2/blobs/sha256/", f.Name()), err)
						//						return err
					}
...
func (bw *blobWriter) Dedup(
	...
	layerPath := path.Join("/home/simenc3/docker_v2", blobPath)
	...
	if(do_partial){
		...
		if(!someWrong){
			...
			// common error: no permission to delete, consider using sudo
			if rerr != nil{
				// create sudo command
				sudocmd := exec.Command("sudo", "-S", "rm", "-rf", unpackPath)
				// 设置 sudo 密码
...
// ========== File-as-Layer: File-level Dedup with Merkle Tree + Partial Decode ==========

// FileDedup implements the "All-Files-As-Layers" approach combined with SimEnc's partial decode.
// Push path: partial decode (./decode) stores FRI file for exact reconstruction,
//            then file-level extraction + dedup + Merkle tree for cross-layer dedup.
// Pull path: partial encode (./encode) reconstructs exact gzip from FRI,
//            with Merkle tree reconstruction as fallback.
func (bw *blobWriter) FileDedup(
	reqtype, reponame, usrname string,
	desc distribution.Descriptor) error {

	fmt.Printf("FileDedup: request type: %s, for repo (%s) and usr (%s) with dgst (%s)\n",
		reqtype, reponame, usrname, desc.Digest.String())
	...
	lfile, err := os.Open(layerPath)
	if err != nil {
		fmt.Printf("simenc: FileDedup cannot open layer file =>%s\n", layerPath)
		return err
	}
	stat, err := lfile.Stat()
	if err != nil {
	...
	if compressedSize == 0 {
		context.GetLogger(ctx).Warnf("FileDedup: empty layer %s", layerPath)
		return nil
	}

	// Update RLMap (same as original Dedup)
	rlmapentry, err := bw.blobStore.registry.metadataService.StatRLMapEntry(ctx, reponame)
	if err == nil {
		if _, ok := rlmapentry.Dgstmap[desc.Digest]; ok {
			// Already processed this layer: file blobs already stored, just clean up original blob dir
			blobPath2, err2 := PathFor(BlobDataPathSpec{Digest: desc.Digest})
			if err2 == nil {
				blobDir2 := path.Dir(path.Join("/home/simenc3/docker_v2", blobPath2))
				if _, serr := os.Stat(blobDir2); serr == nil {
					os.RemoveAll(blobDir2)
				}
			}
			return nil // already processed
		}
		rlmapentry.Dgstmap[desc.Digest] = 1
		if err1 := bw.blobStore.registry.metadataService.SetRLMapEntry(ctx, reponame, rlmapentry); err1 != nil {
			return err1
		}
	} else {
		dgstmap := make(map[digest.Digest]int64)
		dgstmap[desc.Digest] = 1
		rlmapentry = distribution.RLmapEntry{Dgstmap: dgstmap}
		if err1 := bw.blobStore.registry.metadataService.SetRLMapEntry(ctx, reponame, rlmapentry); err1 != nil {
			return err1
		}
	}

	// Step 0: Partial decode - store FRI file for exact gzip reconstruction on pull
	partialBasePath := "/home/simenc3/docker_v2/docker/registry/v2/partials/sha256"
	dgstHex := desc.Digest.Hex()
	partialDir := path.Join(partialBasePath, dgstHex[:2])
	os.MkdirAll(partialDir, 0777)
	partialStorePath := path.Join(partialDir, dgstHex)
	hasPartial := false

	// Check if partial file already exists (from a previous push of the same layer)
	if _, serr := os.Stat(partialStorePath); os.IsNotExist(serr) {
		partialTmpFile := layerPath + "_partial"
		command := "./decode"
		args := []string{layerPath, partialTmpFile}
		cmd := exec.Command(command, args...)
		startPartial := time.Now()
		_, decodeErr := cmd.CombinedOutput()
		partialTime := time.Since(startPartial).Seconds()
		if decodeErr == nil {
			// Move partial file to permanent storage
			if mvErr := os.Rename(partialTmpFile, partialStorePath); mvErr != nil {
				// Rename may fail across filesystems, try copy+delete
				if cpErr := copyFile(partialTmpFile, partialStorePath); cpErr != nil {
					fmt.Printf("FileDedup: cannot store partial file: %v\n", cpErr)
					os.Remove(partialTmpFile)
				} else {
					os.Remove(partialTmpFile)
					hasPartial = true
				}
			...
			fmt.Printf("FileDedup: partial decode OK (%.2fs) hasPartial=%v dgst=%s\n",
				partialTime, hasPartial, desc.Digest.String())
		} else {
			fmt.Printf("FileDedup: partial decode failed: %v (continuing with file extraction)\n", decodeErr)
			os.Remove(partialTmpFile)
		}
	} else {
		hasPartial = true
		fmt.Printf("FileDedup: partial file already exists for %s\n", desc.Digest.String())
	}

	// Step 1: Extract all files from the layer tar.gz
	start := time.Now()
	entries, err := ExtractFilesFromLayer(layerPath)
	if err != nil {
		uploger.Printf("NO%d_FileDedup_Error extract: %v layer_path:%s\n", counter.Increment(), err, layerPath)
		return nil // don't block the push; layer file kept intact
	}
	extractTime := time.Since(start).Seconds()

	if len(entries) == 0 {
		uploger.Printf("NO%d_FileDedup_Empty layer_path:%s\n", counter.Increment(), layerPath)
		return nil
	}

	// Step 2: File-level dedup - store unique file blobs, skip duplicates
	start = time.Now()
	var totalFiles int64 = 0
	var totalSize int64 = 0
	var uniqueFiles int64 = 0
	var dedupFiles int64 = 0
	var dedupSavedBytes int64 = 0

	// fileBlobBasePath is where unique file blobs are stored on disk
	fileBlobBasePath := "/home/simenc3/docker_v2/docker/registry/v2/fileblobs/sha256"
	os.MkdirAll(fileBlobBasePath, 0777)

	for i := range entries {
		entry := &entries[i]
		if entry.IsDir {
			continue // directories are only structural, no blob
		}

		totalFiles++
		totalSize += entry.Size

		// Check if this file blob already exists in Redis
		exists, err := bw.blobStore.registry.metadataService.StatFileBlob(ctx, entry.Digest)
		if err != nil {
			fmt.Printf("FileDedup: StatFileBlob error: %v\n", err)
			continue
		}

		if exists {
			// File content already stored - dedup hit
			dedupFiles++
			dedupSavedBytes += entry.Size
		} else {
			// New unique file - store blob to disk
			uniqueFiles++

			if entry.TypeFlag == tar.TypeReg || entry.TypeFlag == tar.TypeRegA {
				dgstHex := entry.Digest.Hex()
				blobDir := path.Join(fileBlobBasePath, dgstHex[:2])
				os.MkdirAll(blobDir, 0777)
				blobFilePath := path.Join(blobDir, dgstHex)

				if _, serr := os.Stat(blobFilePath); os.IsNotExist(serr) {
					var compBuf bytes.Buffer
					gzw, _ := gzip.NewWriterLevel(&compBuf, gzip.BestSpeed)
					gzw.Write(entry.Content)
					gzw.Close()
					if werr := os.WriteFile(blobFilePath, compBuf.Bytes(), 0644); werr != nil {
						fmt.Printf("FileDedup: write blob error: %v\n", werr)
						continue
					}
				}
			}
			...
			// Record in Redis
			if err := bw.blobStore.registry.metadataService.SetFileBlob(ctx, entry.Digest, entry.Size); err != nil {
				fmt.Printf("FileDedup: SetFileBlob error: %v\n", err)
			}
		}

		// Free content memory after storing
		entry.Content = nil
	}
	dedupTime := time.Since(start).Seconds()

	// Step 3: Build Merkle Tree from directory structure
	start = time.Now()
	merkleRoot, err := bw.buildMerkleTree(ctx, entries)
	if err != nil {
		uploger.Printf("NO%d_FileDedup_Error merkle: %v layer_path:%s\n", counter.Increment(), err, layerPath)
		return nil
	}
	merkleTime := time.Since(start).Seconds()

	// Step 4: Store FileLayerRecipe
	recipe := distribution.FileLayerRecipe{
	...
	if err := bw.blobStore.registry.metadataService.SetFileLayerRecipe(ctx, desc.Digest, recipe); err != nil {
		fmt.Printf("FileDedup: SetFileLayerRecipe error: %v\n", err)
	}

	// Also set a LayerRecipe for backward compatibility with ServeBlob
	layerRecipe := distribution.LayerRecipeDescriptor{
		Digest:            desc.Digest,
		MasterIp:          bw.blobStore.registry.hostserverIp,
		HostServerIps:     []string{bw.blobStore.registry.hostserverIp},
		SliceSizeMap:      map[string]int64{bw.blobStore.registry.hostserverIp: totalSize},
		UncompressionSize: totalSize,
		CompressionSize:   compressedSize,
		Fcnt:              totalFiles,
	}
	bw.blobStore.registry.metadataService.SetLayerRecipe(ctx, desc.Digest, layerRecipe)

	// Step 5: Clean up the original layer blob (data is now stored in file-level blobs)
	// layerPath is ".../blobs/sha256/<2hex>/<hex>/data"
	// We remove the entire blob directory ".../blobs/sha256/<2hex>/<hex>/" to free all space.
	blobDir := path.Dir(layerPath) // ".../blobs/sha256/<2hex>/<hex>"
	if rmErr := os.RemoveAll(blobDir); rmErr != nil {
		uploger.Printf("NO%d_FileDedup_CleanupFailed blobDir:%s err:%v\n",
			counter.Increment(), blobDir, rmErr)
	} else {
		uploger.Printf("NO%d_FileDedup_Cleanup_OK blobDir:%s freedBytes:%d\n",
			counter.Increment(), blobDir, compressedSize)
	}

	dedupRatio := float64(0)
	if totalSize > 0 {
		dedupRatio = float64(dedupSavedBytes) / float64(totalSize) * 100
	}

	uploger.Printf("NO%d_FileDedup_Success hasPartial:%v totalFiles:%d uniqueFiles:%d dedupFiles:%d dedupRatio:%.1f%% "+
		"extractTime:%.2f dedupTime:%.2f merkleTime:%.2f totalSize:%d compressedSize:%d layer_path:%s\n",
		counter.Increment(), hasPartial, totalFiles, uniqueFiles, dedupFiles, dedupRatio,
		extractTime, dedupTime, merkleTime, totalSize, compressedSize, layerPath)

	return nil
}

// buildMerkleTree constructs a Merkle DAG from the extracted file entries.
// The tree mirrors the directory structure of the original tar archive.
// Returns the root digest of the Merkle tree.
func (bw *blobWriter) buildMerkleTree(ctx context.Context, entries []FileEntry) (digest.Digest, error) {
	// Build a virtual directory tree from tar paths
	...
	// Insert each entry into the virtual tree
	for i := range entries {
		entry := &entries[i]
		parts := splitTarPath(entry.TarPath)

		if entry.IsDir {
			// Ensure directory exists in tree
			ensureDir(root, parts)
		} else {
			// Place file in its parent directory
			if len(parts) == 0 {
				continue
			}
			fileName := parts[len(parts)-1]
			dirParts := parts[:len(parts)-1]
			parent := ensureDir(root, dirParts)
			parent.files[fileName] = entry
			parent.order = append(parent.order, "F:"+fileName)
		}
	}

	// Recursively compute Merkle hashes bottom-up
	rootDigest, err := bw.computeMerkleHash(ctx, root)
	if err != nil {
		return "", err
	}
...
// ensureDir creates directory nodes along the path if they don't exist.
func ensureDir(root *dirNode, parts []string) *dirNode {
	current := root
	for _, p := range parts {
		if p == "" || p == "." || p == "/" {
			continue
		...
		if !ok {
			...
			current.order = append(current.order, "D:"+p)
		}
		current = child
	}
	return current
...
// splitTarPath splits a tar path into components.
func splitTarPath(tarPath string) []string {
...
// computeMerkleHash recursively computes the hash for a directory node.
func (bw *blobWriter) computeMerkleHash(ctx context.Context, node *dirNode) (digest.Digest, error) {
	...
	for _, key := range node.order {
		if strings.HasPrefix(key, "D:") {
			...
			childHash, err := bw.computeMerkleHash(ctx, child)
			if err != nil {
				return "", err
			}
			childDigests = append(childDigests, childHash)
		} else if strings.HasPrefix(key, "F:") {
			fileName := key[2:]
			fileEntry := node.files[fileName]

			// Create a leaf Merkle node for this file
			leafNode := distribution.MerkleTreeNode{
				IsDir:      false,
				Name:       fileName,
				FileBlobID: fileEntry.Digest,
				FileSize:   fileEntry.Size,
				FileMode:   fileEntry.Mode,
				LinkTarget: fileEntry.LinkTarget,
			}

			// Compute the node hash: hash of (name + fileBlobID)
			leafHash := ComputeContentDigest([]byte(fmt.Sprintf("file:%s:%s:%d:%d",
				fileName, fileEntry.Digest.String(), fileEntry.Size, fileEntry.Mode)))
			leafNode.NodeHash = leafHash

			// Store in Redis
			if err := bw.blobStore.registry.metadataService.SetMerkleNode(ctx, leafHash, leafNode); err != nil {
				fmt.Printf("FileDedup: SetMerkleNode leaf error: %v\n", err)
			}

			childDigests = append(childDigests, leafHash)
		}
	}

	// Also process children/files not in order (safety net)
	for dirName, child := range node.children {
		found := false
		for _, key := range node.order {
			if key == "D:"+dirName {
				found = true
	...
	for fileName, fileEntry := range node.files {
		found := false
		for _, key := range node.order {
			if key == "F:"+fileName {
				found = true
		...
		if !found {
			...
			bw.blobStore.registry.metadataService.SetMerkleNode(ctx, leafHash, leafNode)
			childDigests = append(childDigests, leafHash)
		}
	}

	// Create the directory Merkle node
	dirMerkleNode := distribution.MerkleTreeNode{
		IsDir:    true,
		Name:     node.name,
		Children: childDigests,
	}

	// Compute directory hash: hash of (name + all child hashes concatenated)
	hashInput := fmt.Sprintf("dir:%s", node.name)
	for _, cd := range childDigests {
		hashInput += ":" + cd.String()
	}
	dirHash := ComputeContentDigest([]byte(hashInput))
	dirMerkleNode.NodeHash = dirHash

	// Store in Redis
	if err := bw.blobStore.registry.metadataService.SetMerkleNode(ctx, dirHash, dirMerkleNode); err != nil {
		fmt.Printf("FileDedup: SetMerkleNode dir error: %v\n", err)
	}
	...
	*fcnt = count
	...
	for no = 1 ; no <= count;no++{
		...
		*dirSize += fsize
		...
		// if decode success, return decoded file descriptor
		if err == nil {
			// dedup hit - delete current file
			err := os.Remove(fpath)
			if err != nil {
				return err
			}
...
func (bw *blobWriter) doDedup(ctx context.Context, desc distribution.Descriptor, unpackPath string, comressSize int64, doPartial bool) (float64, float64, float64, int64, error, bool, bool) {
	...
	if err != nil {
		someWrong = true   // duphunter may fail, need to keep original file
	}
