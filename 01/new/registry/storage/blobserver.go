...
import (
	...
	"github.com/docker2/new/registry/storage/driver"
...
func (bs *blobServer) constructSliceFromPartial(ctx context.Context, desc distribution.SliceRecipeDescriptor,
	...
	// use io.Copy to write buf content to file
...
func (bs *blobServer) constructLayerFromPartial(ctx context.Context, desc distribution.LayerRecipeDescriptor,
	dgst digest.Digest, constructtype string, wg *sync.WaitGroup) ([]byte, float64, string){
		...
		if ok {
			...
			// bss is nil or other error, try to reconstruct
			rbuf.wg.Add(1)
			rbuf.Lock()
			start = time.Now()
			if len(desc.HostServerIps) == 1 {
...
func (bs *blobServer) Preconstructlayers_partial(ctx context.Context, reg *registry) error {
	...
	for elem := range it.C {
		...
		go func( wantdgst digest.Digest, wg *sync.WaitGroup){
			...
			gid := GetGID()
			tmp_id := fmt.Sprintf("%f", gid)
			tmpfilePath := path.Join("/home/simenc3/docker_v2/docker/registry/"  ,tmp_id + strings.SplitN(wantdgst.String(),":",2)[1])
			tmpfile, err := os.Create(tmpfilePath)
			defer os.Remove(tmpfilePath)
			if err != nil{
				context.GetLogger(ctx).Errorf("simenc: os.Create(tmpfilePath) error with %v ", err)
...
// ========== File-as-Layer: Partial Encode Reconstruction ==========

// constructFromPartialEncode reconstructs the exact original gzip layer
// by reading the stored FRI (partial decode) file and running ./encode.
// This produces a byte-for-byte identical gzip stream to the original push.
func (bs *blobServer) constructFromPartialEncode(ctx context.Context, dgst digest.Digest) ([]byte, bool, float64) {
	start := time.Now()

	// Check if partial file exists
	dgstHex := dgst.Hex()
	partialPath := path.Join("/home/simenc3/docker_v2/docker/registry/v2/partials/sha256", dgstHex[:2], dgstHex)

	if _, err := os.Stat(partialPath); os.IsNotExist(err) {
		return nil, false, 0.0
	}

	// Create temp file for encode output
	gid := GetGID()
	tmpId := fmt.Sprintf("%f", gid)
	tmpOutputPath := path.Join("/home/simenc3/docker_v2/docker/registry/", tmpId+dgstHex+".tar.gz")

	command := "./encode"
	args := []string{partialPath, tmpOutputPath}
	cmd := exec.Command(command, args...)
	_, err := cmd.CombinedOutput()
	defer os.Remove(tmpOutputPath)
	if err != nil {
		context.GetLogger(ctx).Warnf("simenc: constructFromPartialEncode: encode failed for %v: %v", dgst, err)
		return nil, false, 0.0
	}

	bss, err := ioutil.ReadFile(tmpOutputPath)
	if err != nil {
		context.GetLogger(ctx).Warnf("simenc: constructFromPartialEncode: read output failed: %v", err)
		return nil, false, 0.0
	}

	if len(bss) == 0 {
		context.GetLogger(ctx).Warnf("simenc: constructFromPartialEncode: empty output for %v", dgst)
		return nil, false, 0.0
	}

	duration := time.Since(start).Seconds()
	context.GetLogger(ctx).Infof("simenc: constructFromPartialEncode: OK size=%d time=%.2fs dgst=%v", len(bss), duration, dgst)
	return bss, true, duration
}

// ========== File-as-Layer: Merkle Tree Reconstruction ==========

// constructFromMerkleTree reconstructs a layer as a tar.gz stream from its Merkle tree.
// It traverses the Merkle DAG, reads file blobs from disk, and assembles them into
// a tar archive with the original directory structure, then gzip-compresses the result.
func (bs *blobServer) constructFromMerkleTree(ctx context.Context, dgst digest.Digest) ([]byte, bool, float64) {
	start := time.Now()

	// Step 1: Get the FileLayerRecipe
	recipe, err := bs.reg.metadataService.StatFileLayerRecipe(ctx, dgst)
	if err != nil {
		context.GetLogger(ctx).Warnf("simenc: constructFromMerkleTree: no FileLayerRecipe for %v: %v", dgst, err)
		return nil, false, 0.0
	}

	// Step 2: Traverse the Merkle tree and build tar entries
	var buf bytes.Buffer
	tw := tar.NewWriter(&buf)

	err = bs.walkMerkleNode(ctx, recipe.MerkleRoot, "", tw)
	if err != nil {
		context.GetLogger(ctx).Errorf("simenc: constructFromMerkleTree: walk error: %v", err)
		tw.Close()
		return nil, false, 0.0
	}

	if err := tw.Close(); err != nil {
		context.GetLogger(ctx).Errorf("simenc: constructFromMerkleTree: tar close error: %v", err)
		return nil, false, 0.0
	}

	// Step 3: Gzip compress
	var compressBuf bytes.Buffer
	gw, _ := pgzip.NewWriterLevel(&compressBuf, 6)
	if _, err := io.Copy(gw, &buf); err != nil {
		context.GetLogger(ctx).Errorf("simenc: constructFromMerkleTree: gzip error: %v", err)
		return nil, false, 0.0
	}
	gw.Close()

	duration := time.Since(start).Seconds()
	return compressBuf.Bytes(), true, duration
}

// walkMerkleNode recursively traverses a Merkle tree node and writes tar entries.
func (bs *blobServer) walkMerkleNode(ctx context.Context, nodeHash digest.Digest, parentPath string, tw *tar.Writer) error {
	node, err := bs.reg.metadataService.StatMerkleNode(ctx, nodeHash)
	if err != nil {
		return fmt.Errorf("cannot find Merkle node %v: %v", nodeHash, err)
	}

	if node.IsDir {
		// Directory node: write directory header, then recurse into children
		dirPath := parentPath
		if node.Name != "/" && node.Name != "" {
			...
			if err := tw.WriteHeader(hdr); err != nil {
				return fmt.Errorf("cannot write dir header for %s: %v", dirPath, err)
			}
		}

		// Recurse into children
		for _, childHash := range node.Children {
			if err := bs.walkMerkleNode(ctx, childHash, dirPath, tw); err != nil {
				return err
			}
		}
	} else {
		// File node: read blob from disk and write to tar
		filePath := path.Join(parentPath, node.Name)

		if node.LinkTarget != "" && node.FileSize == 0 {
			// Symlink
			hdr := &tar.Header{
				Typeflag: tar.TypeSymlink,
				Name:     filePath,
				Linkname: node.LinkTarget,
				Mode:     int64(node.FileMode),
			}
			if err := tw.WriteHeader(hdr); err != nil {
				return fmt.Errorf("cannot write symlink header for %s: %v", filePath, err)
			}
			return nil
		}

		// Regular file: read content from file blob store
		content, err := bs.readFileBlob(ctx, node.FileBlobID)
		if err != nil {
			return fmt.Errorf("cannot read file blob %v for %s: %v", node.FileBlobID, filePath, err)
		}
		...
		if err := tw.WriteHeader(hdr); err != nil {
			return fmt.Errorf("cannot write file header for %s: %v", filePath, err)
		}
		if _, err := tw.Write(content); err != nil {
			return fmt.Errorf("cannot write file content for %s: %v", filePath, err)
		}
	}
...
// readFileBlob reads a file blob from disk (with memory cache).
func (bs *blobServer) readFileBlob(ctx context.Context, fileDgst digest.Digest) ([]byte, error) {
	// Check memory cache first
	dgstStr := fileDgst.String()
	bfss, ok, _ := bs.reg.blobcache.GetFile("fileblob:" + dgstStr)
	if ok {
		return bfss, nil
	}

	// Read from disk
	dgstHex := fileDgst.Hex()
	blobDir := path.Join("/home/simenc3/docker_v2/docker/registry/v2/fileblobs/sha256", dgstHex[:2])
	blobFilePath := path.Join(blobDir, dgstHex)

	// content, err := ioutil.ReadFile(blobFilePath)
	compressed, err := ioutil.ReadFile(blobFilePath)
	if err != nil {
		return nil, fmt.Errorf("cannot decompress file blob %s: %v", blobFilePath, err)
	}
	gr, err := gzip.NewReader(bytes.NewReader(compressed))
	if err != nil {
		return nil, fmt.Errorf("cannot read decompressed file blob %s: %v", blobFilePath, err)
	}
	...
	// Put in cache for future reads
	if len(content) > 0 {
		bs.reg.blobcache.SetFile("fileblob:"+dgstStr, content)
	}
...
func (bs *blobServer) ServeBlob(ctx context.Context, w http.ResponseWriter, r *http.Request, dgst digest.Digest) error {
	...
	DurationMEM := 0.0
	DurationSSD := 0.0   // time to construct layer from unpack files or read from disk
	DurationNTT := 0.0
	...
	if reqtype == "GET" ||reqtype == "LAYER" || reqtype == "PRECONSTRUCTLAYER" || reqtype == "MANIFEST" {
		...
		if ok {
			...
			// Fallback 1: Try partial encode reconstruction (exact gzip from stored FRI)
			bss, cok, DurationSSD = bs.constructFromPartialEncode(ctx, dgst)
			if !cok {
				// Fallback 2: Merkle tree reconstruction (file-level reassembly)
				bss, cok, DurationSSD = bs.constructFromMerkleTree(ctx, dgst)
			}
			if !cok {
				// Fallback 3: SimEnc chunk-level reconstruction (512KiB chunks + partial encode)
				bss, cok, DurationSSD = bs.constructFromUnpackFiles(ctx, dgst, true)
			}
			if !cok {
				// Fallback 4: Read raw layer file directly from disk
			...
			// Cache the reconstructed layer for future pulls
			if len(bss) > 0 {
				bs.reg.blobcache.SetFile(dgst.String(), bss)
			}
