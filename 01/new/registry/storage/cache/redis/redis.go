...
// ========== File-as-Layer: Merkle Tree Redis methods ==========

func (rdms *redisDedupMetadataService) merkleNodeHashKey(nodeHash digest.Digest) string {
	return "MerkleNode::" + nodeHash.String()
}

func (rdms *redisDedupMetadataService) fileLayerRecipeHashKey(layerDigest digest.Digest) string {
	return "FileLayerRecipe::" + layerDigest.String()
}

func (rdms *redisDedupMetadataService) fileBlobHashKey(fileDgst digest.Digest) string {
	return "FileBlob::" + fileDgst.String()
}

func (rdms *redisDedupMetadataService) StatMerkleNode(ctx context.Context, nodeHash digest.Digest) (distribution.MerkleTreeNode, error) {
	reply, err := rdms.cluster.Get(rdms.merkleNodeHashKey(nodeHash)).Result()
	if err == redisgo.Nil {
		return distribution.MerkleTreeNode{}, err
	} else if err != nil {
		context.GetLogger(ctx).Errorf("simenc: StatMerkleNode redis cluster error for key %s", err)
		return distribution.MerkleTreeNode{}, err
	}
	var node distribution.MerkleTreeNode
	if err = node.UnmarshalBinary([]byte(reply)); err != nil {
		context.GetLogger(ctx).Errorf("simenc: StatMerkleNode cannot UnmarshalBinary for key %s", err)
		return distribution.MerkleTreeNode{}, err
	}
	return node, nil
}

func (rdms *redisDedupMetadataService) SetMerkleNode(ctx context.Context, nodeHash digest.Digest, node distribution.MerkleTreeNode) error {
	// Use SetNX for content-addressable: if same hash exists, content is identical
	_, err := rdms.cluster.SetNX(rdms.merkleNodeHashKey(nodeHash), &node, 0).Result()
	if err != nil {
		context.GetLogger(ctx).Errorf("simenc: SetMerkleNode redis cluster cannot set value for key %s", err)
		return err
	}
	return nil
}

func (rdms *redisDedupMetadataService) StatFileLayerRecipe(ctx context.Context, layerDigest digest.Digest) (distribution.FileLayerRecipe, error) {
	reply, err := rdms.cluster.Get(rdms.fileLayerRecipeHashKey(layerDigest)).Result()
	if err == redisgo.Nil {
		return distribution.FileLayerRecipe{}, err
	} else if err != nil {
		context.GetLogger(ctx).Errorf("simenc: StatFileLayerRecipe redis cluster error for key %s", err)
		return distribution.FileLayerRecipe{}, err
	}
	var recipe distribution.FileLayerRecipe
	if err = recipe.UnmarshalBinary([]byte(reply)); err != nil {
		context.GetLogger(ctx).Errorf("simenc: StatFileLayerRecipe cannot UnmarshalBinary for key %s", err)
		return distribution.FileLayerRecipe{}, err
	}
	return recipe, nil
}

func (rdms *redisDedupMetadataService) SetFileLayerRecipe(ctx context.Context, layerDigest digest.Digest, recipe distribution.FileLayerRecipe) error {
	err := rdms.cluster.SetNX(rdms.fileLayerRecipeHashKey(layerDigest), &recipe, 0).Err()
	if err != nil {
		context.GetLogger(ctx).Errorf("simenc: SetFileLayerRecipe redis cluster cannot set value for key %s", err)
		return err
	}
	return nil
}

func (rdms *redisDedupMetadataService) StatFileBlob(ctx context.Context, fileDgst digest.Digest) (bool, error) {
	exists, err := rdms.cluster.Exists(rdms.fileBlobHashKey(fileDgst)).Result()
	if err != nil {
		context.GetLogger(ctx).Errorf("simenc: StatFileBlob redis cluster error for key %s", err)
		return false, err
	}
	return exists > 0, nil
}

func (rdms *redisDedupMetadataService) SetFileBlob(ctx context.Context, fileDgst digest.Digest, size int64) error {
	_, err := rdms.cluster.SetNX(rdms.fileBlobHashKey(fileDgst), size, 0).Result()
	if err != nil {
		context.GetLogger(ctx).Errorf("simenc: SetFileBlob redis cluster cannot set value for key %s", err)
		return err
	}
	return nil
}
