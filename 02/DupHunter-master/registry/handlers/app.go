...
func NewApp(ctx context.Context, config *configuration.Configuration) *App {
	...
	ctxu.GetLogger(app).Infof("NANNAN: Register blobDispatcher")
...
func (app *App) configureRedis(configuration *configuration.Configuration) {
	...
	for _, ip := range configuration.Notifications.Registries {
		if "192.168.7.5" == ip {
			redisdb = redisgo.NewClusterClient(&redisgo.ClusterOptions{
				Addrs: []string{
					"192.168.7.5:7000", "192.168.7.5:7001",
				}})
			redisdb.Ping()
			ok, err := redisdb.FlushAll().Result()
			fmt.Println(ok, err)
			fmt.Println("==========> THIS IS TESTING SINGLE PC ============> \n")
			found = true
			break
		}
	...
	if false == found {
		...
		ok, err := redisdb.FlushAll().Result()
		fmt.Println(ok, err)
		fmt.Println("==========> THIS IS TESTING SINGLE PC FALLBACK ============> \n")
	}
